...
function SettingsPage() {
  ...
  const contacts = useQuery({
    queryKey: ["contacts"],
    queryFn: async () =>
      (await supabase.from("emergency_contacts").select("*").order("created_at")).data ?? [],
  });
  ...
  useEffect(() => {
    if (profile.data) {
      const displayName =
        typeof window !== "undefined" ? window.localStorage.getItem("lifeline.displayName") : null;
      setFullName(profile.data.full_name || displayName || "");
      setPhone(profile.data.phone ?? "");
      setBlood(profile.data.blood_type ?? "");
      setAllergies((profile.data.allergies ?? []).join(", "));
      setMeds((profile.data.medications ?? []).join(", "));
    }
  }, [profile.data]);
  ...
  const addContact = useMutation({
    mutationFn: async () => {
      ...
      const { error } = await supabase
        ...
        .insert({ user_id: userData.user.id, name: cName, relation: cRel, phone: cPhone });
      if (error) throw error;
    },
    onSuccess: () => {
      setCName("");
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-border bg-card p-6">
          ...
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>{t("dash.bloodType")}</Label>
                <Input value={blood} onChange={(e) => setBlood(e.target.value)} placeholder="O+" />
              </div>
              <div>
                ...
                <div className="mt-1.5 flex gap-1">
                  {(["en", "sn", "nd"] as const).map((l) => (
                    <button
                      ...
                      className={`flex-1 rounded-md border px-2 py-1.5 text-xs ${appLang === l ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"}`}
                    >
                      {l.toUpperCase()}
                    </button>
                  ))}
        ...
        <section className="rounded-2xl border border-border bg-card p-6 lg:col-span-2">
          ...
          <div className="space-y-4">
            <Row label="Dark mode" desc="Softer palette for low-light environments">
              <Switch checked={darkMode} onCheckedChange={setDarkMode} />
            </Row>
            <Row label="Push notifications" desc="Emergency alerts and hospital acceptance">
              <Switch defaultChecked />
            </Row>
            <Row label="SMS fallback" desc="Send SMS when data is unavailable">
              <Switch defaultChecked />
            </Row>
            <Row label="Two-factor authentication" desc="Coming soon, extra layer for your account">
              <Button variant="outline" size="sm" disabled>
                <Shield className="mr-1 h-4 w-4" /> Set up
              </Button>
            </Row>
            <Row label="Delete account" desc="Permanently remove your LifeLine+ account and data">
