    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .select("id,severity,status,created_at,eta_minutes,ai_report,symptoms")
  ...
  const kpis = [
    {
      ...
      value:
        q.data?.filter((r) =>
          ["hospital_notified", "dispatched", "en_route", "transporting"].includes(r.status),
        ).length ?? 0,
    },
  ...
  return (
    <AppShell>
      <div className="mb-6 flex items-center gap-2">
        ...
        <h1 className="font-display text-2xl font-semibold md:text-3xl">{t("hospital.title")}</h1>
      </div>
