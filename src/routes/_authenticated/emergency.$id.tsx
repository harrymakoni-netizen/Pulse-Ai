...
const FLOW = ["requested","assessed","hospital_notified","dispatched","en_route","arrived","transporting","completed"] as const;
...
function EmergencyDetail() {
  ...
  const req = useQuery({
    ...
    queryFn: async () => {
      const [{ data: r }, { data: events }] = await Promise.all([
        supabase.from("emergency_requests").select("*, hospital:hospitals(name,city,phone,address)").eq("id", id).single(),
  ...
  const advance = useMutation({
    mutationFn: (status: "dispatched"|"en_route"|"arrived"|"transporting"|"completed"|"cancelled") =>
  ...
  useEffect(() => {
    const r = req.data?.request; if (!r) return;
    const idx = FLOW.indexOf(r.status as typeof FLOW[number]);
    if (idx < 0 || idx >= FLOW.indexOf("completed")) return;
    const nextStatuses = ["dispatched","en_route","arrived","transporting","completed"] as const;
    const currentIdx = nextStatuses.findIndex(s => s === r.status);
    // if just hospital_notified -> dispatch after 4s
    const delay = r.status === "hospital_notified" ? 4000 : r.status === "dispatched" ? 6000 : r.status === "en_route" ? 10000 : r.status === "arrived" ? 6000 : r.status === "transporting" ? 8000 : 0;
    if (delay === 0) return;
    const next = r.status === "hospital_notified" ? "dispatched" : nextStatuses[currentIdx + 1];
    if (!next) return;
    const t = setTimeout(() => { advance.mutate(next); }, delay);
                ...
                return (
                  <li key={step} className="flex gap-3">
                    <div className="flex flex-col items-center">
                      <div className={`flex h-8 w-8 items-center justify-center rounded-full border-2 ${done ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground"} ${isCurrent ? "pulse-calm" : ""}`}>
