...
function AmbulancePage() {
  ...
  return (
    <AppShell>
      <div className="mb-6 flex items-center gap-2"><Ambulance className="h-5 w-5 text-primary" /><h1 className="font-display text-2xl font-semibold md:text-3xl">{t("amb.title")}</h1></div>
        ...
        {[
          ...
          <div key={c.id} className="rounded-2xl border border-border bg-card p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-2">
              <button className="rounded-md border border-border px-3 py-2 text-xs hover:bg-secondary flex items-center justify-center gap-1"><Navigation className="h-3 w-3" /> {t("amb.navigate")}</button>
              <button className="rounded-md bg-primary px-3 py-2 text-xs text-primary-foreground hover:bg-primary/90 flex items-center justify-center gap-1"><PlayCircle className="h-3 w-3" /> {t("amb.accept")}</button>
            </div>
          </div>
        ))}
      </div>
