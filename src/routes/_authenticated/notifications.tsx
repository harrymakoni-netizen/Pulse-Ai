  ...
  return (
    <AppShell>
      <div className="mb-6 flex items-center gap-2">
        ...
        <h1 className="font-display text-2xl font-semibold md:text-3xl">{t("notif.title")}</h1>
      </div>
      {q.data?.length ? (
        <ul className="divide-y divide-border rounded-2xl border border-border bg-card">
          {q.data.map((n) => (
            <li key={n.id} className="flex items-start gap-3 p-4">
              <div
                className={`mt-0.5 flex h-8 w-8 items-center justify-center rounded-full ${n.category === "emergency" ? "bg-[color:var(--alert)]/10 text-[color:var(--alert)]" : "bg-primary/10 text-primary"}`}
              >
