...
import { Accessibility, X, Plus, Minus, Sun, Moon, Contrast, Type, RotateCcw, PauseCircle, MousePointer2, Link as LinkIcon, Eye, BookOpenText } from "lucide-react";
...
export function AccessibilityWidget() {
  ...
  useEffect(() => {
    ...
    function onMove(e: MouseEvent) { setGuideY(e.clientY); }
    ...
    <>
      ...
      {settings.readingGuide ? (
        <div
          ...
          style={{ top: guideY - 20, background: "rgba(225,29,72,0.12)", borderTop: "2px solid rgba(225,29,72,0.6)", borderBottom: "2px solid rgba(225,29,72,0.6)" }}
        />
      ) : null}
        ...
        >
          ...
          >
            <div className="sticky top-0 flex items-center justify-between border-b border-border bg-background/95 px-5 py-4 backdrop-blur">
              ...
              <button type="button" onClick={() => setOpen(false)} aria-label="Close accessibility menu" className="rounded-full p-2 hover:bg-secondary">
            ...
            <div className="space-y-6 px-5 py-5">
              <Group title="Reading">
                <Stepper
                  ...
                  onDec={() => update("textScale", Math.max(0.85, +(settings.textScale - 0.1).toFixed(2)))}
                  onInc={() => update("textScale", Math.min(1.8, +(settings.textScale + 0.1).toFixed(2)))}
                />
                <Stepper
                  label={`Letter spacing · ${settings.letterSpacing.toFixed(2)}em`}
                  onDec={() => update("letterSpacing", Math.max(0, +(settings.letterSpacing - 0.02).toFixed(2)))}
                  onInc={() => update("letterSpacing", Math.min(0.2, +(settings.letterSpacing + 0.02).toFixed(2)))}
                />
                <Stepper
                  label={`Line spacing · ${settings.lineHeight.toFixed(2)}`}
                  onDec={() => update("lineHeight", Math.max(1.2, +(settings.lineHeight - 0.1).toFixed(2)))}
                  onInc={() => update("lineHeight", Math.min(2.4, +(settings.lineHeight + 0.1).toFixed(2)))}
                />
              </Group>

              <Group title="Display">
                <Toggle icon={<Contrast className="h-4 w-4" />} label="High contrast" checked={settings.highContrast} onChange={(v) => update("highContrast", v)} />
                <Toggle icon={settings.darkMode ? <Moon className="h-4 w-4" /> : <Sun className="h-4 w-4" />} label="Dark mode" checked={settings.darkMode} onChange={(v) => update("darkMode", v)} />
                <Toggle icon={<Type className="h-4 w-4" />} label="Dyslexia-friendly font" checked={settings.dyslexiaFont} onChange={(v) => update("dyslexiaFont", v)} />
                <Toggle icon={<LinkIcon className="h-4 w-4" />} label="Highlight links & buttons" checked={settings.highlightLinks} onChange={(v) => update("highlightLinks", v)} />
                <Toggle icon={<MousePointer2 className="h-4 w-4" />} label="Larger cursor" checked={settings.bigCursor} onChange={(v) => update("bigCursor", v)} />
              </Group>

              <Group title="Motion & focus">
                <Toggle icon={<PauseCircle className="h-4 w-4" />} label="Pause animations" checked={settings.reduceMotion} onChange={(v) => update("reduceMotion", v)} />
                <Toggle icon={<BookOpenText className="h-4 w-4" />} label="Reading guide (follows cursor)" checked={settings.readingGuide} onChange={(v) => update("readingGuide", v)} />
                <Toggle icon={<Eye className="h-4 w-4" />} label="Reading mask (dim edges)" checked={settings.readingMask} onChange={(v) => update("readingMask", v)} />
              </Group>
...
function Stepper({ icon, label, onInc, onDec }: { icon?: React.ReactNode; label: string; onInc: () => void; onDec: () => void }) {
  return (
    <div className="flex items-center justify-between gap-3 rounded-lg border border-border bg-card px-3 py-2">
      ...
      <div className="flex items-center gap-1">
        <button type="button" onClick={onDec} aria-label={`Decrease ${label}`} className="rounded-md border border-border p-1.5 hover:bg-secondary"><Minus className="h-3.5 w-3.5" /></button>
        <button type="button" onClick={onInc} aria-label={`Increase ${label}`} className="rounded-md border border-border p-1.5 hover:bg-secondary"><Plus className="h-3.5 w-3.5" /></button>
      </div>
    </div>
  );
}
...
function Toggle({ icon, label, checked, onChange }: { icon?: React.ReactNode; label: string; checked: boolean; onChange: (v: boolean) => void }) {
  return (
    ...
    >
      ...
      <span className={`inline-flex h-5 w-9 items-center rounded-full transition-colors ${checked ? "bg-[color:var(--alert)]" : "bg-border"}`}>
